...
public class CustomLiveAction implements LiveAction {
	...
	public void execute(Resource source, Resource target, LiveRelationship liveRelationship, boolean autoSave,
		...
		// Determine if the live action is configured to copy the cq:lastModifiedBy
		// property
		if ((boolean) configs.get("repLastModBy")) {

			// Get cq:lastModified of the source
			if (source != null && source.adaptTo(Node.class) != null) {
				ValueMap sourceVM = source.adaptTo(ValueMap.class);
				lastModified = sourceVM.get(NameConstants.PN_PAGE_LAST_MOD_BY, String.class);
			}

			// Set the target's la-lastModifiedBy property
			Session session = null;
			if (target != null && target.adaptTo(Node.class) != null) {
				// Getting the resource resolver from the resource object
				...
				try {
					...
					log.info("***** Target node's last modified by property is updated: {}", lastModified);
				} catch (Exception e) {
					log.error(e.getMessage());
				}
			}

			// Save the values in the repository via session when autoSave is true
			if (autoSave) {
				try {
					session.save();
				} catch (Exception e) {
